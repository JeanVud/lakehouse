...
START_DATE = START_DATE = pendulum.datetime(2023, 4, 2, tz="Asia/Ho_Chi_Minh")
# How often to Run. @daily - Once a day at non busy pipeline running time
SCHEDULE_INTERVAL = "7 14 * * *"
# Who is listed as the owner of this DAG in the Airflow Web Server
DAG_OWNER_NAME = "Admin"
# List of email address to send email alerts to if this job fails
# ALERT_EMAIL_ADDRESSES = ['tam.nguyen3@mservice.com.vn']
ALERT_EMAIL_ADDRESSES = ['vinh.dang@mservice.com.vn']
# Length to retain the log files if not already provided in the conf. If this
# is set to 30, the job will remove those files that arE 30 days old or older.

DEFAULT_MAX_DB_ENTRY_AGE_IN_DAYS = int(
    Variable.get("airflow_db_cleanup__max_db_entry_age_in_days", 15)
)
...
def cleanup_function(**context):
  ...
  try:
    ...
    print_deletes = context["params"].get("print_deletes", DEFAULT_PRINT_DELETES)
    ...
    if print_deletes:
      ...
      # Limit number of entry log to airflow UI
      entry_count = 0
      for entry in entries_to_delete:
        ...
        entry_count += 1
        if entry_count >= 1000:
          logging.info("... (Only show first 1000 entry)")
          break
